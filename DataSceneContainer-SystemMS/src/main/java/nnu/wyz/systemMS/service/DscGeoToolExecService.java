...
public class DscGeoToolExecService {
    ...
    void execute(DscGeoToolExecTask dscGeoToolExecTask) {
        ...
        String[] params = this.getExecCommand(executor, toolName, dscGeoToolExecTask.getParams(), (List<Map<String, Object>>) toolParams.get("parameters"), outputFileRecords);
...
//                            log.info(logs);
            ...
            if (!Objects.equals(errMsg, "")) {
                log.error(errMsg);
                stopTask(dscGeoToolExecTask, errMsg);
                return;
            }
            //TODO: 输出文件入库
